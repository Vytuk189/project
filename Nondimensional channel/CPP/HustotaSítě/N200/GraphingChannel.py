...
data = []
file_path = 'flowdata_channel_nondimensional_N200_iter500000_CFL05_beta1.000000.txt'
...
plt.title('Horizontální rychlost U při síti ' + str(N-3) + "x" + str(M-1), fontsize=16, pad=12)
plt.xlabel('x')
plt.ylabel('y')
...
plt.title('Vertikální rychlost V při síti ' + str(N-3) + "x" + str(M-1), fontsize=16, pad=12)
plt.xlabel('x')
plt.ylabel('y')
plt.gca().set_aspect('equal', adjustable='box')  # Ensure axes are proportional
# Plot the second mesh, only where binary_data is 0, set color to white
...
plt.title('Tlak P při síti ' + str(N-3) + "x" + str(M-1), fontsize=16, pad=12)
plt.xlabel('x')
plt.ylabel('y')
plt.gca().set_aspect('equal', adjustable='box')  # Ensure axes are proportional
# Plot the second mesh, only where binary_data is 0, set color to white
...
# Load the data
data_time = np.loadtxt('times_channel_nondimensional_CPP_N200_iter500000_CFL05_beta1.000000.txt')
data_res = np.loadtxt('residues_channel_nondimensional_CPP_N200_iter500000_CFL05_beta1.000000.txt')
...
plt.title('Rezidua při síti ' + str(N-3) + "x" + str(M-1), fontsize=16, pad=12)
ax1.legend()
...
ax2.set_xticks(iterations[::50000])  # Change this step as needed

# Round the simulation time to one decimal place for tick labels
rounded_time = np.round(data_time[::50000], 1)  # Round the time values
