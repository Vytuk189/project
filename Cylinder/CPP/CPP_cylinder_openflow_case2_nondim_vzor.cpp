...
The boundary conditions for velocity in this specific case are Dirichlet on the inflow, Neumann everywhere else DNN. Pressure has a Neumann boundary condition on every boundary
    ...
    while (counter < iter) {
        ...
        for (int i = 1; i < N + 2; ++i) {
            for (int j = 1; j < M; ++j) {
                ...
                // Multiply the result with 1 or 0 based on whether the point is in flow or in cylinder
