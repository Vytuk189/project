...
data = []
file_path = 'flowdata_cylinder_nondimensional_N400_Re10.000000_iter0.txt'
...
plt.streamplot(X, Y, u_speeds_grid, v_speeds_grid, color='black', linewidth=0.7, density=2, integration_direction='both', arrowstyle = '-', minlength=0.2)
plt.xlabel('x')
plt.ylabel('y')
...
# # Exclude these points by setting the velocity components to NaN
# u_speeds_grid_masked = np.where(mask, np.nan, u_speeds_grid)
# v_speeds_grid_masked = np.where(mask, np.nan, v_speeds_grid)

# # Plot the streamline visualization with the masked velocity field
# plt.figure(figsize=(10, 5))
# plt.streamplot(X, Y, u_speeds_grid_masked, v_speeds_grid_masked, color='black', linewidth=0.7, density=8, integration_direction='both', arrowstyle = '-', minlength=1)
# plt.xlabel('X Coordinate')
# plt.ylabel('Y Coordinate')
# plt.title('Streamline Visualization Excluding the Cylinder Region')
# cmap_binary = ListedColormap(["none", "black"])
# #plt.pcolormesh(X, Y, mask, shading='auto', cmap=cmap_binary)
# plt.grid(True)
