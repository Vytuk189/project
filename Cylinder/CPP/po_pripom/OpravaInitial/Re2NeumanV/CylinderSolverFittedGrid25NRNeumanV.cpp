...
const double min_node_density_inlet = 1; //inlet
...
Matrix CreateMeshFreeFlow() {
    ...
    double delta_y = delta_y_min + (y_zone_number_top+1)*(delta_y_max-delta_y_min)/vert_zone_count;
    ...
    delta_y = delta_y_min + (y_zone_number_bottom+1)*(delta_y_max-delta_y_min)/vert_zone_count;

    while(y_temp > 0+0.5*delta_y_max) {
        y_coordinates.insert(y_coordinates.begin(), y_temp);
        if(y_temp < y_dense_top + (y_zone_number_bottom+1)*(H-y_dense_bottom)/vert_zone_count){
            y_temp = y_temp - delta_y;
        }
        else {
            y_zone_number_bottom++;
            delta_y = delta_y_min + (y_zone_number_bottom)*(delta_y_max-delta_y_min)/vert_zone_count;
            y_temp = y_temp - delta_y;
        }
    }
    ...
    while (counter < iter) {
       ...
       //tau = 0.0001;
        ...
        for (int j = 0; j < M; ++j) {
            ...
            // Vertical speed also doesnt change
