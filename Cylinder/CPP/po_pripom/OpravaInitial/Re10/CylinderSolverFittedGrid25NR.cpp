...
const double min_node_density = 1; //outlet
...
const double x_dense_right = center_x + 2.5*R;
...
Matrix CreateMeshFreeFlow() {
    ...
    while(y_temp > 0+0.5*delta_y_max) {
        y_coordinates.insert(y_coordinates.begin(), y_temp);
        if(y_temp < y_dense_bottom + (y_zone_number_bottom+1)*(H-y_dense_bottom)/vert_zone_count){
            y_temp = y_temp - delta_y;
        }
        else {
