...
const double min_node_density = 0.75; //outlet
const double min_node_density_inlet = 2.5; //inlet
const double min_node_density_walls = 4;
...
const int right_zone_count = 20;
const int vert_zone_count = 20;
...
Matrix CreateMeshFreeFlow() {
    ...
    while(x_temp < x_dense_left-0.5*delta_x_min) {
    ...
    x_temp = x_dense_left;
    while(x_temp < x_dense_right-0.5*delta_x_min) {
        x_coordinates.push_back(x_temp);
    ...
    while(x_temp < L-0.5*delta_x_min) {
        x_coordinates.push_back(x_temp);
    ...
    while(y_temp < y_dense_bottom-0.5*delta_y_min) {
    ...
    while(y_temp < y_dense_top-0.5*delta_y_min) {
        y_coordinates.push_back(y_temp);
    ...
    while(y_temp < H-0.5*delta_x_min) {
        y_coordinates.push_back(y_temp);
...
void saveMatrixWithTime(const Matrix& past, int iter_number, double time) {
    std::string filename = "DFGRe100_NR"+std::to_string(max_node_density)+"_Iter"+ std::to_string(iter_number) + ".txt";
    std::ofstream file(filename);
    file << time << std::endl;
    for (const auto& row : past) {
        for (const auto& col : row) {
...
void saveResidues(const std::vector<std::vector<double>>& residues, int iter_number) {
    std::string filename = "residues_DFGRe100_NR"+std::to_string(max_node_density)+"_Iter"+ std::to_string(iter_number) + ".txt";
    std::ofstream file(filename);
