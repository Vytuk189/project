...
for i in range(N+3):
    for j in range(M+1):
        ...
        #Initializes the channel to the analytical solution
        #Could also be initialized to the calculated numerical solution for flow
        #between plates without an obstacle - almost identical
        p = p_in - (p_in-p_out)/L*x
        u = parabolic_function(y, p_in - p_out, L, rho, Re, H)
