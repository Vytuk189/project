...
const double omega = 3.14159265359*RPM/30;
...
Matrix createInitialMatrix(const Matrix& coordinates) {
    ...
    for (int i = 0; i < N; ++i) {
        for (int j = 0; j < M; ++j) {
            ...
            if(fluid_this == 0){
                double x = initial_matrix[i][j][3];
                double y = initial_matrix[i][j][4];
                double r_now = std::sqrt((x - closest_to_center_x) * (x - closest_to_center_x) + (y - closest_to_center_y) * (y - closest_to_center_y));
                double sin_theta = (y-closest_to_center_y)/r_now;
                double cos_theta = (x-closest_to_center_x)/r_now;
                initial_matrix[i][j][1] = omega*r_now*sin_theta;
                initial_matrix[i][j][2] = -omega*r_now*cos_theta;
            }
       ...
       //tau = 0.0001;
        ...
        for (int i = 1; i < N - 1; ++i) {
            for (int j = 1; j < M - 1; ++j) {
                ...
                if(a == 0) {
                    ...
                    double x = initial_matrix[i][j][3];
                    double y = initial_matrix[i][j][4];
                    double r_now = std::sqrt((x - center_x) * (x - center_x) + (y - center_y) * (y - center_y));
                    predict[i][j][1] = omega*r_now*ny;
                    predict[i][j][2] = -omega*r_now*nx;
        ...
        for (int i = 1; i < N - 1; ++i) {
            for (int j = 1; j < M - 1; ++j) {
                ...
                double y = initial_matrix[i][j][4];
                double r_now = std::sqrt((x - center_x) * (x - center_x) + (y - center_y) * (y - center_y));
                past[i][j][1] = omega*r_now*ny;
                past[i][j][2] = -omega*r_now*nx;
