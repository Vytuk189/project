...
void specialSaveMatrixWithTime(const Matrix& past, int special_counter, double time) {
    std::string filename = "DFGPeriodic_DataPart_Counter"+std::to_string(special_counter)+".txt";
    std::ofstream file(filename);
    file << time << std::endl;
    for (const auto& row : past) {
        for (const auto& col : row) {
...
int main(){
    ...
    while (counter < iter) {
       ...
       //tau = 0.0001;
        ...
        if(time > 27){
            if(special_counter % 10 == 0){
                specialSaveMatrixWithTime(past, special_counter, time);
            }
            special_counter++;
        }

        if(special_counter == 10000){
            break;
        }
