...
data = []
file_path = 'flowdata_cylinder_NR25.000000_Re20.000000_Iter500000.txt'
...
mask = (structure_grid == 0) | np.isnan(structure_grid)
# Plot the grid
...
M = len(y_vals)
DN = 80
plt.title("Síť " + str(N) + "x" + str(M) + ", $N_{D}$ = " + str(DN))
plt.xlabel("X")
plt.ylabel("Y")
plt.grid(False)
...
plt.title('Re = 2: Horizontální rychlost U')
plt.xlabel('x')
plt.ylabel('y')
...
plt.title('Re = 2: Vertikální rychlost V')
plt.xlabel('x')
plt.ylabel('y')
plt.gca().set_aspect('equal', adjustable='box')  # Ensure axes are proportional
# Plot the second mesh, only where binary_data is 0, set color to white
...
plt.colorbar(label='V')
plt.title('Re = 2: Celková rychlost')
plt.xlabel('x')
plt.ylabel('y')
plt.gca().set_aspect('equal', adjustable='box')  # Ensure axes are proportional
# Plot the second mesh, only where binary_data is 0, set color to white
...
cmap_binary = ListedColormap(["none", "white"])
#plt.pcolormesh(X, Y, mask, shading='auto', cmap=cmap_binary)
# Add a horizontal colorbar under the plot
cbar = plt.colorbar(label='P', orientation='horizontal')  # Set orientation to horizontal
cbar.ax.set_xlabel('P')  # Optional: Add a label for the colorbar
plt.show()
...
# Load the data
data_res = np.loadtxt('residues_cylinder_NR25.000000_Re20.000000_Iter500000.txt')
data_time = np.loadtxt('times_channel_nondimensional_CPP_DR25.000000_iter500000_CFL05_beta1.000000.txt')
...
plt.title('Laminární test: Rezidua', fontsize=16, pad=12)
ax1.legend()
...
ax2.set_xticks(iterations[::100000])  # Change this step as needed

# Round the simulation time to one decimal place for tick labels
rounded_time = np.round(data_time[::100000], 1)  # Round the time values
...
print("Rozdíl tlaků je" + str(diff))
