...
% Open the file
filename = 'flowdata_cylinder_NR25.000000_Re20.000000_Iter500000.txt';
...
[x_grid, y_grid] = meshgrid(linspace(x_min, x_max, 400), linspace(y_min, y_max, 400));
...
% figure;
% contourf(x_grid, y_grid, pressure_grid, 20, 'LineColor', 'none');
% hold on;
% % contour(x_grid, y_grid, pressure_grid, 20, 'LineColor', 'k');
% colormap(jet);
% cb = colorbar;
% cb.Color = 'k';
% cb.FontSize = 12;
...
% y_limits = ylim;
% 
% % Set the aspect ratio based on the data range
% aspect_ratio = (x_limits(2) - x_limits(1)) / (y_limits(2) - y_limits(1));
% 
% % Set the aspect ratio to be equal to the data range ratio
% axis equal;
% pbaspect([aspect_ratio 1 1]);
% 
% xlabel('X', 'Color', 'k', 'FontWeight', 'bold', 'FontSize', 14);
% ylabel('Y', 'Color', 'k', 'FontWeight', 'bold', 'FontSize', 14);
% 
% t = title('Re = 2: Tlak P', 'Color', 'k', 'FontWeight', 'bold', 'FontSize', 30);
% drawnow; % ensure rendering happens before moving title
% set(t, 'Units', 'normalized', 'Position', [0.5, 1.05, 0]);
% 
% % Set the axis properties to remove the top and right ticks
% ax = gca;
% ax.Box = 'off';  % Turn off the box to remove the top and right borders
% ax.XAxisLocation = 'bottom';  % Ensure the X-axis is at the bottom
% ax.YAxisLocation = 'left';    % Ensure the Y-axis is on the left
% 
% % Disable the ticks on the top and right
% ax.XTickLabel = {}; % Remove X ticks on the top
% ax.YTickLabel = {}; % Remove Y ticks on the right
% 
% xlabel('X', 'Color', 'k', 'FontWeight', 'bold', 'FontSize', 14);
% ylabel('Y', 'Color', 'k', 'FontWeight', 'bold', 'FontSize', 14);
% 
% t = title('Laminární případ: Tlak P', 'Color', 'k', 'FontWeight', 'bold', 'FontSize', 30);
% drawnow; % ensure rendering happens before moving title
% set(t, 'Units', 'normalized', 'Position', [0.5, 1.05, 0]);
% set(gcf, 'Color', 'w');
% set(gca, 'Color', 'w', 'XColor', 'k', 'YColor', 'k', 'TickDir', 'out', 'FontSize', 12);
...
% ylabel('Y', 'Color', 'k', 'FontWeight', 'bold', 'FontSize', 14);
% 
% t = title('Laminární případ: Horizontální rychlost U', 'Color', 'k', 'FontWeight', 'bold', 'FontSize', 30);
% drawnow; % ensure rendering happens before moving title
% set(t, 'Units', 'normalized', 'Position', [0.5, 1.05, 0]);
% set(gcf, 'Color', 'w');
% set(gca, 'Color', 'w', 'XColor', 'k', 'YColor', 'k', 'TickDir', 'out', 'FontSize', 12);
...
t = title('Laminární případ: Vertikální rychlost V', 'Color', 'k', 'FontWeight', 'bold', 'FontSize', 30);
drawnow; % ensure rendering happens before moving title
set(t, 'Units', 'normalized', 'Position', [0.5, 1.05, 0]);
set(gcf, 'Color', 'w');
set(gca, 'Color', 'w', 'XColor', 'k', 'YColor', 'k', 'TickDir', 'out', 'FontSize', 12);
...
t = title('Laminární případ: Celková rychlost', 'Color', 'k', 'FontWeight', 'bold', 'FontSize', 30);
drawnow; % ensure rendering happens before moving title
set(t, 'Units', 'normalized', 'Position', [0.5, 1.05, 0]);
set(gcf, 'Color', 'w');
set(gca, 'Color', 'w', 'XColor', 'k', 'YColor', 'k', 'TickDir', 'out', 'FontSize', 12);
...
starty = linspace(0.05, 0.35 , n);  % Seed points for streamlines in y-direction


% Cylinder parameters
cx = 0.5;   % X-coordinate of the cylinder center
...
t = title('Laminární případ: Proudnice', 'Color', 'k', 'FontWeight', 'bold', 'FontSize', 20);
drawnow; % ensure rendering happens before moving title
set(t, 'Units', 'normalized', 'Position', [0.5, 1.05, 0]);
