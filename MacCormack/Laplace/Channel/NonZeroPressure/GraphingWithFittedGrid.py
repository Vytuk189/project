...
data = []
file_path = 'flowdata_cylinder_N100_Re10.000000_Iter190000.txt'
...
DN = 80
plt.xlim(0,2)
plt.title("Síť 100x100 pro kanál s IBM")
plt.xlabel("X")
plt.ylabel("Y")
plt.grid(False)
...
cmap_binary = ListedColormap(["none", "red"])
...
plt.title('Horizontální rychlost U pro kanál s IBM')
plt.xlabel('x')
plt.ylabel('y')
...
plt.title('Vertikální rychlost U pro kanál s IBM')
plt.xlabel('x')
plt.ylabel('y')
plt.gca().set_aspect('equal', adjustable='box')  # Ensure axes are proportional
# Plot the second mesh, only where binary_data is 0, set color to white
...
plt.title('Tlak P pro kanál s IBM')
plt.xlabel('x')
plt.ylabel('y')
plt.gca().set_aspect('equal', adjustable='box')  # Ensure axes are proportional
# Plot the second mesh, only where binary_data is 0, set color to white
...
data_res = np.loadtxt('residues_cylinder_N100_Re10.000000_Iter390000.txt')
data_time = np.loadtxt('times_channel_nondimensional_CPP_N100_iter390000_CFL05_beta1.000000.txt')
...
plt.title('Rezidua pro kanál s IBM')
ax1.legend()
