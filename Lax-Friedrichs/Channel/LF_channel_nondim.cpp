    ...
    while (counter < iter) {
        ...
        for (int i = 1; i < N + 2; ++i) {
            for (int j = 1; j < M; ++j) {
                ...
                double new_p = (1-zeta)*p_this + zeta*(p_right + p_up + p_left + p_down)/4 + rho*beta*beta*tau*(-central_first_u_x-central_first_v_y);
                double new_u = (1-zeta)*u_this + zeta*(u_right + u_up + u_left + u_down)/4 + tau*( -u_this*central_first_u_x - v_this*central_first_u_y - 1/rho*central_first_p_x + nu*(central_second_u_x + central_second_u_y));
                double new_v = (1-zeta)*v_this + zeta*(v_right + v_up + v_left + v_down)/4 + tau*( -u_this*central_first_v_x - v_this*central_first_v_y - 1/rho*central_first_p_y + nu*(central_second_v_x + central_second_v_y));
