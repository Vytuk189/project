...
const double center_x = 0.4*L;
const double center_y = 0.5*H;
const double R = 0.2*H;


const double nu = (u_in*2*R)/Re;
...
const double rate_of_step_increase = 1.2;
...
Matrix CreateMeshFreeFlow() {
    ...
    while(y_temp < H-delta_y/2) {
        y_coordinates.push_back(y_temp);
        delta_y = rate_of_step_increase*delta_y;
        y_temp = y_temp + delta_y;
    }

    y_coordinates.push_back(H);

    delta_y = rate_of_step_increase*delta_y_min;
       ...
       //tau = 0.0001;
        ...
        for (int i = 1; i < N - 1; ++i) {
            for (int j = 1; j < M - 1; ++j) {
                ...
                predict[i][j][0] = (1-zeta)*p_this + zeta*(p_right + p_up + p_left + p_down)/4 + rho*beta*beta*tau*(-central_first_u_x-central_first_v_y);
                predict[i][j][1] = (1-zeta)*u_this + zeta*(u_right + u_up + u_left + u_down)/4 + tau*( -u_this*central_first_u_x - v_this*central_first_u_y - 1/rho*central_first_p_x + nu*(central_second_u_x + central_second_u_y));
                predict[i][j][2] = (1-zeta)*v_this + zeta*(v_right + v_up + v_left + v_down)/4 + tau*( -u_this*central_first_v_x - v_this*central_first_v_y - 1/rho*central_first_p_y + nu*(central_second_v_x + central_second_v_y));

                sums[0] += (predict[i][j][0] - past[i][j][0])*(predict[i][j][0] - past[i][j][0]);
                sums[1] += (predict[i][j][1] - past[i][j][1])*(predict[i][j][1] - past[i][j][1]);
                sums[2] += (predict[i][j][2] - past[i][j][2])*(predict[i][j][2] - past[i][j][2]);
                continue;
                }
                ...
                if(a == 0) {
                    ...
                    sums[0] += (predict[i][j][0] - old_p_here)*(predict[i][j][0] - old_p_here);
                    continue;
                }
